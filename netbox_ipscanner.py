...
class IpScan(Script):
    ...
    class Meta:
        name = "IP 扫描器"
        description = "扫描可用前缀并更新 IPAM 中的 IP 地址及其状态"
        job_timeout = 12000  # 脚本超时时间，单位为秒。防止因地址过多，导致脚本超时。
    ...
    def run(self, data, commit):
        ...
        nb.http_session.verify = False # 禁用Netbox的https证书检查

        subnets = nb.ipam.prefixes.all()  #提取netbox里面的所有前缀，格式为 x.x.x.x/yyy

        for subnet in subnets:
            if str(subnet.status) == 'Reserved': # 不扫描状态为保留的前缀
